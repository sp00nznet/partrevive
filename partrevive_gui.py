...
class PartReviveGUI:
    ...
    def _set_busy(self, busy):
        ...
        if busy:
            self.plan_btn.config(state="disabled"); self.restore_btn.config(state="disabled")
    ...
    def _add_cand_row(self, c):
        ...
        self.ctree.insert("", "end", tags=(tag,), values=(
            c.confidence, c.fstype, f"{c.start:,}", human(c.size), c.typecode, label))

    def _scan_done(self):
        self._set_busy(False)
        live = sum(1 for c in self.cands if c.confidence == "mounted")
        self.prog["value"] = 100
